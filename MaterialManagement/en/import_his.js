...
.datepicker({
    ...
    $("#btnModify").click(function () {
        ...
        if ($("#tblfinal input:checkbox:checked").length > 0) {
            ...
            alert("You have to select a row to modify!");
            return false;
        }
    });
    //import save data
                    ...
                    if (txtPrice == "") {
                        ...
                        if (isNaN(txtPrice)) {
                            ...
                            if (parseFloat(txtPrice) <= 0) {
                                alert("Price is bigger zero!");
                                $('#txtPrice').focus();
                                return false;
                            }
                            else
                                if (Imp_date == "") {
                                    alert("Please input import date!");
                                    $('#txtImpDate').focus();
                                    return false;
                                }
                                else {
                                    ...
                                    $.ajax({
                                        ...
                                        data: JSON.stringify({
                                            Seq: Seq, QCode: QCode, Imp_date: Imp_date, txtQty: txtQty, txtPrice: txtPrice, Po: Po, txtImp_Supplier: txtImp_Supplier, txtImp_Buyer: txtImp_Buyer, txtImp_Receiver: txtImp_Receiver, OldQCode: OldQCode, OldImp_Date: OldImp_Date, GET: GET, Remark: Remark
                                        }),
                                        type: 'POST',
                                        dataType: 'json',
                                        contentType: 'application/json; charset=utf-8',
    ...
    $("#btnDelete").click(function () {
        if ($("#tblfinal input:checkbox:checked").length > 0) {
            ...
            var confirm1 = confirm("Do you want to delete this row have QCode= " + Qcode + " không?");
            if (confirm1) {
                $.ajax({
