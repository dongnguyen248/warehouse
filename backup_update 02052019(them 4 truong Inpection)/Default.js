  ...
  //$("#tbMainDefault").on('click', 'tr', function (e) {
    ...
    $("#btnallocate").click(function () {       
        if ($("#tbMainDefault input:checkbox:checked").length > 0) {
            ...
            var locator = tr.children("td:nth-child(12)").text();
            $("#hdprice").val(price);
            $("#txtQcode").val(QCode);
    ...
    $("#btnImport").click(function () {
        if ($("#tbMainDefault input:checkbox:checked").length > 0) {
            ...
            var locator = $.trim(tr.children("td:nth-child(12)").text());
            alert(locator);
            ...
            if (txtPrice == "")
            {
                ...
                if (Imp_date == "")
                {
                    ...
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            QCode: QCode, Imp_date: Imp_date, txtQty: txtQty, txtPrice: txtPrice,Po:Po, txtImp_Supplier: txtImp_Supplier, txtImp_Buyer: txtImp_Buyer, txtImp_Receiver: txtImp_Receiver,Get:Get,locator:locator
                        }),
                        type: 'POST',
                        dataType: 'json',
                        contentType: 'application/json; charset=utf-8',
    ...
    $('#frmModify').submit(function (event) {
        ...
        if (action == 0) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    QCode: QCode, Zone: Zone, Location: Location, Item: Item, Spec: Spec, Unit: Unit, Price: Price,Po:Po, Qty: Qty, Remark: Remark, Pur_Date: Pur_Date, Supplier: Supplier, Buyer: Buyer, Receiver: Receiver,Get:Get,locator:locator
                }),
                type: 'POST',
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
